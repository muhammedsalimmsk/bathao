...
class CallTracker {
  ...
  void onRoomStateChanged(ZegoUIKitRoomState state) async {
    debugPrint('📡 Room state changed: ${state.reason.name} at ${DateTime.now()}');

    try {
      if (state.reason == ZegoRoomStateChangedReason.Logined) {
        if (!_isCallActive) { // Ensure call start logic runs only once
          _isCallActive = true;
          debugPrint('✅ Call started - Initializing call data');

          // Ensure receiverId and callType are set before starting the call
          if (receiverId == null || controller.callType == null) {
            debugPrint("❌ Error: receiverId or callType is null when logging in.");
          ...
          _minuteTimer = Timer.periodic(const Duration(minutes: 1), (timer) async {
            ...
            if (controller.callId == null) {
              debugPrint("⚠️ Call ID is null during minute check. Ending timer.");
              _clearTimers();
              return;
            }
            ...
            if (balance == null || balance < 100) {
              debugPrint("⚠️ Low balance ($balance). Ending call in 59 seconds.");
              ...
              _preCutoffTimer = Timer(const Duration(seconds: 59), () {
                debugPrint("⛔ Call auto-ended due to insufficient balance. Triggering hangUp.");
                if (_isCallActive) { // Only end call if it's still considered active
                  controller.endCall(controller.callId!).then((_) {
        ...
        debugPrint('❌ Call ended by ZegoCloud - reason: ${state.reason.name} at ${DateTime.now()}');
        _clearTimers();
  ...
  void onCallEnd(ZegoCallEndEvent event, void Function() defaultAction) async {
    debugPrint("❌ Call ended — reason: ${event.reason.name} at ${DateTime.now()}");
    _clearTimers();
    _isCallActive = false; // Reset active flag
  ...
  void _clearTimers() {
    ...
    debugPrint('Timers cleared. Minute timer null: ${_minuteTimer == null}, Pre-cutoff timer null: ${_preCutoffTimer == null}');
  }
}